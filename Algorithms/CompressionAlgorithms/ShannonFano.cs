...
{
    public class ShannonFano:ICompressionAlgorithm
    {
        ...
        private int _CompressInternal(byte[] input, byte[] output, uint inputSize)
        {
        ...
        private static void _DecompressInternal(byte[] input, byte[] output, uint inputSize, uint outputSize)
        {
        ...
        {
            uint originalDataSize = (uint)message.Length;
            byte[] compressedData = new byte[originalDataSize * (101 / 100) + 384 + 2*sizeof(int)];

            int compressedDataSize = _CompressInternal(message, compressedData, originalDataSize);

            var originalSizeBytes = BitConverter.GetBytes(originalDataSize);
            var compressedSizeBytes =  BitConverter.GetBytes(compressedDataSize);

            compressedData = compressedData.Take(compressedDataSize).Concat(originalSizeBytes).ToArray();
            return compressedData;
        }

        public byte[] Decompress(byte[] message)
        {
            var data = message.Take(message.Length - 4);
            var size = BitConverter.ToUInt32(message.Skip(message.Length - 4).Take(4).ToArray(), 0);

            byte[] decompressedData = new byte[size];
            _DecompressInternal(message, decompressedData, (uint)message.Length, size);
            Console.WriteLine($"\nOriginal size:{size}\nCompressed size: {message.Length}\n");
